...
(defun compile-fc (foo start &optional brk)
     ...
     (setf foundfunc nil)
      ...
      (when (equal "CUSTOMCHARS" (elt foo i))
        (format o " lda #24~%")
        (format o " sta $D018~%")
      )

      (when (equal "DEFAULTCHARS" (elt foo i))
        (format o " lda #20~%")
        (format o " sta $D018~%")
      )
      ...
      (when (equal "LOADCHAR" (elt foo i))
        (setf temp1 #x2000)
        (format o " ldx #0~%")
        (format o (concatenate 'string " jmp LoadDataNo" (write-to-string label-count) "~%"))
        (format o (concatenate 'string "CharDataNo" (write-to-string label-count) "~%"))
        (incf i)
        (setf temp1 (+ temp1 (* (parse-integer (elt foo i)) 8)))
        (incf i)
        (setf bytes 0)
        (format o " byte ")
        (loop
          (when (equal ")" (elt foo i))(return))
          (format o "#~d" (elt (get-byte (elt foo i)) 0))
          (if (< bytes 7)
            (format o ",")
            (format o "~%")
          )
        ...
        (format o (concatenate 'string " lda " (concatenate 'string "CharDataNo" (write-to-string label-count))))
        (format o ",x~%")
